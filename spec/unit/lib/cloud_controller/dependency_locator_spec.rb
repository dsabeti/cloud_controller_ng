  ...
  describe '#package_blobstore' do
    ...
    context('when bits service is enabled') do
      ...
      it 'creates the client with the right arguments' do
        expect(CloudController::Blobstore::ClientProvider).to receive(:provide).with(options: config[:packages], directory_key: 'key', resource_type: :packages, bits_client: kind_of(BitsClient))
        locator.package_blobstore
      end
    end
  end
